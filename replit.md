...
- Idle-loop videos: optional 5s seamless boomerang loops (`sN-stepM-*-loop.mp4`) that play under the question/feedback UI to keep scenes feeling "alive". Loop file = reverse(last 2.5s) + forward(last 2.5s) of source video, audio-stripped, libx264 crf 22, faststart. Mapping in `STEP_VIDEO_LOOPS` (server/seed.ts); `ensureStepLoopVideos()` populates `scenario_steps.loop_video_url` at startup (idempotent). Currently shipped only for Scenario 1 (Sports Injury) steps 1,2,3,4,5,7. Trainer renders a separate `<video data-testid="video-idle-loop">` that always autoplays muted+loop+playsInline; opacity toggles 0/1 based on `showLoopVideo = (phase==="question"||"feedback") && !!loopVideoUrl`. Loop element uses ref callback to set muted synchronously (avoids React's autoPlay-before-muted race). Note: H.264 video playback can NOT be verified via Playwright Chromium (codec missing); always test loops in a real browser.
