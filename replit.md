...
- Payments are wired through real **Stripe Checkout** via the Replit Stripe connector (`server/stripeClient.ts`). `POST /api/organizations` creates a pending org + a Stripe Checkout Session and returns `checkoutUrl`; the client redirects to it. Codes + `status=active` are generated only by `checkout.session.completed` webhook delivery in `server/webhookHandlers.ts`. The webhook route is registered in `server/index.ts` BEFORE `express.json()` (raw body required for signature verification). `stripe-replit-sync` manages the webhook endpoint + secret + Stripe→Postgres `stripe` schema sync; `initStripe()` runs `runMigrations` + `findOrCreateManagedWebhook` + `syncBackfill` on startup. Fulfillment is concurrency-safe: `markOrganizationPaid` only updates rows where `status <> 'active'` and returns the row only if the transition won, so redelivered webhooks can never double-generate codes. Dashboard polls `/api/organizations/:id` every 2s while `status != active` (after Stripe redirects back with `?paid=1`).
